...
//        String memKey = aSi + "," + bSi;
    ...
    public static int longestIncreasingSubsequence(int[] nums, int prevMax, int currIndex,
                                                   HashMap<String, Integer> mem) {
        if (currIndex == nums.length) return 0;

        String key = prevMax + ", " + currIndex;
        if (mem.containsKey(key)) return mem.get(key);

        int takeCurrent = 0;
        if (nums[currIndex]
                > prevMax)
            takeCurrent = 1 + longestIncreasingSubsequence(nums, nums[currIndex], currIndex + 1, mem);
        int skipCurrent = longestIncreasingSubsequence(nums, prevMax, currIndex + 1, mem);

        mem.put(key, Math.max(takeCurrent, skipCurrent));
        return mem.get(key);
    }

    public static int longestIncreasingSubsequenceDp(int[] arr) {
        int[] dp = new int[arr.length];
        Arrays.fill(dp, 1); // Each num is a valid subsequence of len 1

        for (int curr = 1; curr < dp.length; curr++) {
            for (int prev = 0; prev < curr; prev++) { // compare with all previous
        ...
        for (int len : dp) {
            maxSubSeq = Math.max(maxSubSeq, len);
        }
        return maxSubSeq;
    }
}
